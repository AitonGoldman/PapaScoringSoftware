        ...
        try:            
            ...
            if existing_queue:
                ...
                if os.getenv('FCM_API_KEY') and len(players_to_alert)>0:                    
                    ...
                    for player_to_alert in players_to_alert:
                        if player_to_alert['token'] is None:
                            continue
                        position = nth(player_to_alert['position'])
                        msg = "You're queue position for %s is now %s%s" % (player['machine_name'],player_to_alert['position'],nth(player_to_alert['position']))
                        if player_to_alert['position']==1:
                            msg = "You are next to play %s" % (player_to_alert['machine_name']) 
                        
                        send_indiv_message.delay(title,msg,player_to_alert['token'])                
                if audit_log_action:                    
                    audit_log_params={
