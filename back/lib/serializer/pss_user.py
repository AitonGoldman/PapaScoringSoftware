...
def generate_pss_user_to_dict_serializer(type_of_serializer):
    def serialize_full_pss_user(pss_user_model):
        ...
        event_serializer = event.generate_event_to_dict_serializer(event.MINIMUM_EVENT)
        ...
        serialized_pss_user['events']=[event_serializer(event_model) for event_model in pss_user_model.events]
