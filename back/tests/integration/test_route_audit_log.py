...
class RouteAuditLogTD(td_integration_test_base.TdIntegrationDispatchTestBase):
    def setUp(self):
        ...
        for division in self.flask_app.tables.Division.query.all():
            ...
            self.new_machine = self.flask_app.tables.Machine.query.filter_by(machine_id=machine_id).first()
        ...
        # add score (player)
        # add score (team)
    def test_audit_log_get_range(self):
        ...
        with self.flask_app.test_client() as c:
            ...
            metadivision_machine = self.flask_app.tables.DivisionMachine.query.filter_by(division_id=6).first()
            rv = c.put('/division/6/division_machine/%s/player/1'%metadivision_machine.division_machine_id)            
            rv = c.post('/entry/division_machine/%s/score/1234'%metadivision_machine.division_machine_id)            
            ...
            #rv = c.get('/admin/audit_log/where_all_my_scores_at/player_id/1/audit_log_id/7/time_delta/1')
            rv = c.get('/admin/audit_log/where_all_my_scores_at/player_id/1/audit_log_id/21/time_delta/5')                        
    ...
    #         rv = c.get('/admin/audit_log/where_all_my_tokens_at/player_id/1')
    #         audit_log = json.loads(rv.data)['data']
    #         self.assertRegexpMatches(audit_log[0],"Purchased on \d+-\d+-\d+ \d+\:\d+\:\d+\.\d+ -  for division Main, A - sold by test_admin - number purchased : 5, remaining tokens :  Main, A : 5 .  Split Flipper : 5 .  Classics : 5 .")
    #         self.assertRegexpMatches(audit_log[1],"Purchased on \d+-\d+-\d+ \d+\:\d+\:\d+\.\d+ -  for metadivision Classics - sold by test_admin - number purchased : 5, remaining tokens :  Main, A : 5 .  Split Flipper : 5 .  Classics : 5 .")
