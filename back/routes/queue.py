...
def set_player_id_based_on_type_of_login(user,input_data):
    if hasattr(current_user,'pss_user_id'):        
        is_player=False
...
def clear_tournament_queue(tables,tournament_id):
    queue_groups = queue_helpers.get_queue_for_tounament(current_app,tournament_id)
    with tables.db_handle.session.no_autoflush:                
        try:
            queues_to_lock_for_for_removal = current_app.tables.Queues.query.with_for_update().join(current_app.tables.TournamentMachines).filter_by(tournament_id=tournament_id).all()
            for tournament_machine_id, queue_group in queue_groups:
                for queue in queue_group:
                    queue.player_id=None
                    queue.bumped=None
            tables.db_handle.session.commit()
            return jsonify({'result':'clear!'})            
        except Exception as e:            
            tables.db_handle.session.commit()
            return jsonify({'result':'internal error : %s' % e,'added_queue':{}})
...
def remove_player_from_queue(tables,tournament_machine_id):
    ...
    player_id = set_player_id_based_on_type_of_login(current_user,input_data)    
    check_tournament_machine_and_player(current_app,tournament_machine_id,player_id)       

    with tables.db_handle.session.no_autoflush:                
        try:                                           
            
            queues = queue_helpers.get_queue_for_tounament_machine(current_app,tournament_machine_id)
            result = queue_helpers.remove_player_from_queue(current_app,player_id,queues=queues)
            tables.db_handle.session.commit()            
            if result:
            ...
            return jsonify({'result':'player removed','updated_queue':[queue_serializer(queue) for queue in queues]})                            
        ...
        try:                                           
            if action == 'bump':                                
                ...
                if queues[0].bumped is False and queues[1].player_id is not None:                    
                    queue_helpers.bump_player_down_queue(current_app,tournament_machine_id,queues)
                    tables.db_handle.session.commit()
                    
                elif queues[1].player_id is None or queues[0].bumped is True:                
                    queue_helpers.remove_player_from_queue(current_app,
                                                           queues[0].player_id,
                                                           queues=queues,
                                                           position_in_queue=1)
                    ...
                    tables.db_handle.session.commit()
                queue_serializer = serializer.queue.generate_queue_to_dict_serializer(serializer.queue.ALL)                
                return jsonify({'result':'player bumped','updated_queue':[queue_serializer(queue) for queue in queues if queue.player_id is not None]})                
        except Exception as e:            
            tables.db_handle.session.commit()
            return jsonify({'result':'internal error : %s' % e,'added_queue':{}})
    ...
    with tables.db_handle.session.no_autoflush:                
        try:                                           
            ...
            if existing_queue:                
                ...
                queues = queue_helpers.get_queue_for_tounament_machine(current_app,tournament_machine_id_to_remove_from)

                queue_helpers.remove_player_from_queue(current_app,
                                                       player_id,
                                                       queues=queues,
                                                       position_in_queue=existing_position)
                if current_app.event_config['ionic_api_key']:                    
...
def get_tournament_machine_queue(tables,tournament_machine_id):
    ...
    queues = queue_helpers.get_queue_for_tounament_machine(current_app,tournament_machine_id)
    queue_serializer = serializer.queue.generate_queue_to_dict_serializer(serializer.queue.ALL)                
    return jsonify({'tournament_machine_queue':[queue_serializer(queue) for queue in queues if queue.player_id is not None]})                
