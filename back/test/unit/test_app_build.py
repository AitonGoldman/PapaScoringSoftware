...
#FIXME : need to re-factor unit tests to use magickmock properly (i.e. test just the unit, not the things it calls)
class AppBuildTest(PssUnitTestBase):    
    ...
    def generate_throw_exception_for_set_event_config_from_db(self):
        def throw_exception(arg1):
            raise Exception('event poop does not exist')
    ...
    def test_get_base_app_for_nonexistent_event(self):
        ...
        app.tables.Events.query.all.return_value=fake_events
        mock_pss_config = MagicMock()
        mock_pss_config.set_event_config_from_db=self.generate_throw_exception_for_set_event_config_from_db()
    ...
    def test_get_base_app_with_nonexistent_flask_secret_key(self):
        ...
        app.tables.Events.query.all.return_value=fake_events
        mock_pss_config = MagicMock()         
        #FIXME : this should not rely on exceptions but should use magickmocks properly
        #with self.assertRaises(Exception) as cm:
        #    app_build.get_base_app(app,mock_pss_config)                    
    ...
    def test_get_event_app_with_existing_event(self):
        ...
        mock_pss_config = MagicMock()
        mock_pss_config.pss_admin_event_name='poop'
