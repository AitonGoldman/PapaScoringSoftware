...
def getTournamentResultsQuery(tournament_id,signifigant_scores):
    ...
    query = "select sum(machine_rank), player_id, tournament_id from (select machine_rank,tournament_machine_id,player_id,tournament_id,row_number() over (partition by player_id,tournament_id order by tournament_id,machine_rank desc) as row from (select tournament_id,player_id,score,tournament_machine_id,papa_scoring_func(rank() over (partition by tournament_machine_id order by score desc)) as machine_rank from (select score,player_id,tournament_machine_id,tournament_id from (select scores.score_id,scores.score,scores.player_id,scores.tournament_machine_id,scores.tournament_id,row_number() over (partition by scores.player_id, scores.tournament_machine_id, scores.tournament_id order by score desc) as p from scores,tournament_machines where scores.tournament_id=%s and scores.voided!=true and scores.tournament_machine_id=tournament_machines.tournament_machine_id and tournament_machines.removed=false) as sub where p=1) as sub_two) as sub_three order by player_id) as sub_four where row <= %s group by player_id,tournament_id" % (tournament_id, signifigant_scores)       
    return query

def getTournamentMachineResultsQuery(tournament_id=None, tournament_machine_id=None):
    where_string = "where scores.tournament_id= %s and scores.tournament_machine_id=tournament_machines.tournament_machine_id and tournament_machines.removed!=true and scores.voided!=true" % tournament_id
    if tournament_machine_id:
        where_string = where_string + " and scores.tournament_machine_id = %s"  % tournament_machine_id
    ...
    tournament_machines_query = "select machine_rank,tournament_machine_id,player_id,tournament_id,score, row_number() over (partition by player_id,tournament_id order by tournament_id,machine_rank desc) as row from (select tournament_id,player_id,score,tournament_machine_id,papa_scoring_func(rank() over (partition by tournament_machine_id order by score desc)) as machine_rank from (select score, sub.player_id,sub.tournament_machine_id,sub.tournament_id from (select score_id,score,scores.player_id,scores.tournament_machine_id,scores.tournament_id,row_number() over (partition by scores.player_id, scores.tournament_machine_id, scores.tournament_id order by score desc) as p from scores,tournament_machines %s) as sub where p=1) as sub_two) as sub_three order by machine_rank desc" % where_string    
