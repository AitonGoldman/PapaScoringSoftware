...
class RouteMetaTournamentTest(pss_integration_test_base.PssIntegrationTestBase):
    def setUp(self):
        super(RouteMetaTournamentTest,self).setUp()        
    ...
    def test_meta_tournament_create_with_event_creator(self):        
        test_tournament_name="test_tournament"+self.create_uniq_id()
        post_dict = {"tournament":{"tournament_name":test_tournament_name}}        
        tournaments_dict_1 = self.login_and_create_tournament(self.admin_login_dict,post_dict,self.event_id)
        test_tournament_name="test_tournament"+self.create_uniq_id()
        post_dict = {"tournament":{"tournament_name":test_tournament_name}}
        tournaments_dict_2 = self.login_and_create_tournament(self.admin_login_dict,post_dict,self.event_id)

        tournament_id_1 = tournaments_dict_1['data'][0]['tournament_id']
        tournament_id_2 = tournaments_dict_2['data'][0]['tournament_id']

        post_dict = {"meta_tournament_name":"test_meta_tournament_name",
                     "tournament_ids":[tournament_id_1,tournament_id_2]}        

        meta_tournament = self.login_and_create_meta_tournament(self.admin_login_dict,post_dict,self.event_id)
        self.assertEquals(meta_tournament['data']['meta_tournament_name'],'test_meta_tournament_name')
        tournament_in_db = self.test_app.table_proxy.Tournaments.query.filter_by(tournament_id=tournament_id_1).first()
        self.assertEquals(tournament_in_db.meta_tournament_id,meta_tournament['data']['meta_tournament_id'])
        tournament_in_db = self.test_app.table_proxy.Tournaments.query.filter_by(tournament_id=tournament_id_2).first()
        self.assertEquals(tournament_in_db.meta_tournament_id,meta_tournament['data']['meta_tournament_id'])

    def test_meta_tournament_edit_stripe(self):        
        ...
        meta_tournament = self.login_and_create_meta_tournament(self.admin_login_dict,post_dict,self.event_id)                        
        post_dict={"meta_tournament_id":meta_tournament['data']['meta_tournament_id'],"use_stripe":True,"stripe_sku":test_sku,"discount_stripe_sku":test_discount_sku}
        meta_tournament_dict = self.login_and_edit_meta_tournament(self.admin_login_dict,post_dict,self.event_id)
        self.assertTrue(meta_tournament_dict['data']['stripe_price'] is not None)
        self.assertTrue(meta_tournament_dict['data']['discount_stripe_price'] is not None)
